...
public class SQLController {
	...
	public void disconnect() {
		try {
			...
			selectUserByListingBooked.close();
			deleteUser.close();
			insertListing.close();
			selectAllListing.close();
			selectListingAddr.close();
			selectListingPostalCode.close();
			selectListingBooked.close();
			deleteListing.close();
			deleteListingHostedBySIN.close();
			insertHosts.close();
			selectHostsBySIN.close();
			insertAvailability.close();
			insertOrUpdateAvailability.close();
			updateAvailability.close();
			updateAvailabilityPrice.close();
			updateAvailabilityBetweenDate.close();
			selectAllAvailBetweenDate.close();
			selectAvailBetweenDate.close();
			sumAvailBetweenDate.close();
			deleteAvailBetweenDate.close();
			getMinMaxPrice.close();
			insertBooked.close();
			updateBookedPriceByBIN.close();
			updateBookedUpdatedBySIN.close();
			cancelBooked.close();
			cancelBookedByListing.close();
			cancelBookedByHostedListing.close();
			selectAllBookedBySIN.close();
			selectBookedBySIN.close();
			selectAllBookedByHostListings.close();
			selectBookedByHostListings.close();
			selectBookedByAddress.close();
			selectBookedByAddressDate.close();
			selectBookedUpdatedBySIN.close();
			insertCancellation.close();
			reportNumBookingsCity.close();
			reportNumBookingsPostalCode.close();
			reportRenterBooking.close();
			reportRenterBookingCity.close();
			reportNumListingsCount.close();
			reportNumListingsCountCity.close();
			reportNumListingsCountCityPost.close();
			reportRankHostCount.close();
			reportRankHostCountCity.close();
			reportNumCancelled.close();
			reportHost10Percent.close();
			createLargestCancelView.close();
			insertComment.close();
			insertCommentOnListing.close();
			insertCommentOnUser.close();
			selectCommentByListing.close();
			selectCommentByUser.close();
			selectCommentMadeByUser.close();
			selectAllListingWComments.close();
			conn.close();
		} catch (SQLException e) {
